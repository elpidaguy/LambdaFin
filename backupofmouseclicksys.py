...
def uploadclientlist():
	...
	for upload in request.files.getlist("file"):
		...
		# df.AccountNo.fillna(0).astype(int)
		# df.AccountNo2.astype(str)
		# df.AccountNo3.fillna(0).astype(int)
		...
		for i in df.index:
			...
			if clientlist!=None:
				...
				# familyheadstatus = mongo.db.familylist.find_one({"Family":df['Family'][i]})
				# print familyheadstatus
				# if familyheadstatus == None:
				# 	mongo.db.clientlist.insert({"ClientName":str(df['ClientName'][i]),"Family":str(df['Family'][i]),"UCC":str(df['UCC'][i]),"Bank1":str(df['Bank1'][i]),"Acc1":str(df['AccountNo'][i]),"Bank2":str(df['Bank2'][i]),"Acc2":str(df['AccountNo2'][i]),"Bank3":str(df['Bank3'][i]),"Acc3":str(df['AccountNo3'][i]),"familyhead":True})
				# 	mongo.db.familylist.insert({"ClientName":str(df['ClientName'][i]),"Family":str(df['Family'][i]),"UCC":str(df['UCC'][i]),"Bank1":str(df['Bank1'][i]),"Acc1":str(df['AccountNo'][i]),"Bank2":str(df['Bank2'][i]),"Acc2":str(df['AccountNo2'][i]),"Bank3":str(df['Bank3'][i]),"Acc3":str(df['AccountNo3'][i]),"username":"","password":""})				
				# else:
...
@app.route("/testreadexcel",methods=["POST"])
def testreadexcel():
	df = pd.read_excel('amcs.xlsx', sheet_name='Sheet1')	 
	print("Column headings:")
	print(df.columns)
	for i in df.index:
		amc = mongo.db.amcs.find_one({"SchemeCode":df['SchemeCode'][i]})
		print amc
		if amc!=None:
			mongo.db.amcs.update_one({"_id":ObjectId(amc['_id'])} ,{"$set":{"NAV":df['NAV'][i]}},upsert=False)
			print("already present")
		else:
			mongo.db.amcs.insert({"AMC":df['AMC'][i],"Category":df['Category'][i],"SchemeCode":df['SchemeCode'][i],"SchemeName":df['SchemeName'][i],"NAV":df['NAV'][i]})
			print("not present insert")
...
def getsliplesscheques():
	slipless = mongo.db.chqueIn.find({"Slip":"","finas":True,"DepositedDate": { "$ne" : ""}},{"_id":False})
...
def getDishonChqs():
	dishon = mongo.db.chqueIn.find({"dishon":True},{"_id":False})
...
def getAssignedCheques():
	finas = mongo.db.chqueIn.find({"finas":True},{"_id":False})
	headers=["Family","UCC","Bank","AccountNo","ChqNo","Amount","DepositedDate","Slip","Comments"]
	finas = list(finas)
	# print finas
	if len(finas):
...
def getChqIns():
	chqinlist = mongo.db.chqueIn.find({'finas':False},{'Slip':False,'DepositedDate':False})
...
def exportswitchdone():
	...
	switchdonelist = mongo.db.switchout.find({'SwitchDate':date},{"_id":False,"SOUnits":False,"SONAV":False})
...
def exportTodays():
	...
	todaylist = mongo.db.today.find({'BuyDate':data['todaydate']},{'Slip':False,"_id":False})
...
def getTodays():
	todaylist = mongo.db.today.find({},{'Slip':False})
	todaylist=list(todaylist)
	if len(todaylist):
		for x in todaylist:
			x["_id"]=str(x["_id"])
...
def getDaywiseliquid():
	liquidlist = mongo.db.daywise.find({'liquid':True,'dep':'b'})
...
def addnewchqin():
	...
	if ifexists == None :
		mongo.db.chqueIn.insert({"DepositedDate":"","Bank":str(formdata['Bank']),"UCC":str(ucc),"AccountNo":str(formdata['AccountNo']),"Family":str(formdata['Family']),"ChqNo":str(chqno),"Amount":str(formdata['Amount']),"Comments":str(formdata['Comments']),"preas":False,"finas":False,"dishon":False,"Slip":""})
...
def finalassigncheque():
	...
	for x in formdata['daywiseuccdata']:
		...
		if x.get('values') != None:
			if x['values'] != '':
				mongo.db.today.insert({"AMC":str(x['AMC']),"UCC":str(formdata['UCC']),"Family":str(formdata['Family']),"Category":str(x['Category']),"ChqNo":str(formdata['ChqNo']),"Amount":str(x['values']),"SchemeCode":str(x['SchemeCode']),"SchemeName":str(x['SchemeName']),"Folio":str(x['Folio']),"OrderNo":"","BuyDate":str(buydate.strftime('%d-%b-%y'))})
